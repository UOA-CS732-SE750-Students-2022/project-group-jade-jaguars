...
describe('Events', () => {
  it.only('Get', async (done) => {
    ...
    expect(eventGetResponse.body.id).to.equal(eventId);
    done();
  });
  ...
  it('Create', async () => {
    ...
    expect(await EventModel.exists({ _id: createResponse.body.id })).to.exist;
  });
  ...
  it('Patch', async () => {
    ...
    expect(eventUpdateResponse.body.title).to.equal('changed');
  });
  ...
  it('Delete', async () => {
    ...
    expect(await EventModel.exists({ _id: eventId })).to.be(null);
  });

  it('Generate random identifier', async () => {
    expect(identifier(10)).to.have.length(10);
  });
  ...
  describe('Search', () => {
    ...
    it('By teamId', async () => {
      ...
      expect(searchResponse[0].id).to.equal(eventId.toString());
    });
